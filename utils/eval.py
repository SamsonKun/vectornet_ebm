...
def get_eval_metric_results_ebm(model, data_loader, device, out_channels, max_n_guesses, horizon, miss_threshold):
    """
    ADE, FDE, and Miss Rate
    """
    forecasted_trajectories, gt_trajectories = {}, {}
    ...
    for data in data_loader:
        ...
        # single gpu testing
            ...
            data = data.to(device)
            gt = data.y.view(-1, out_channels).to(device)
        plan_recon, ftraj = model.forward(data)
        interpolated_future = model.predict(ftraj, plan_recon)
        for i in range(gt.size(0)):
            pred_y = interpolated_future[i].view((-1, 2)).detach().cpu().numpy()
            y = gt[i].view((-1, 2)).cumsum(axis=0).detach().cpu().numpy()
            forecasted_trajectories[seq_id] = [pred_y]
            gt_trajectories[seq_id] = y
            seq_id += 1
    metric_results = get_displacement_errors_and_miss_rate(
